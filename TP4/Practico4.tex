...
\begin{extrabox}
Si tenés disponible el código fuente del núcleo, buscá \texttt{\_\_schedule()}
en el directorio \texttt{kernel/sched/}. Compará su organización con la
descripción de \texttt{man 7 sched}. La ubicación y los detalles internos
cambian entre versiones; el objetivo es reconocer la separación entre la
interfaz de usuario y la implementación del planificador.
\end{extrabox}
...
El intérprete de comandos distingue entre el primer plano (\emph{foreground}),
que ocupa la terminal, y el segundo plano (\emph{background}), que permite
volver al prompt. El número de trabajo sólo tiene sentido dentro de ese shell;
el PID identifica al proceso en el sistema.
...
mediciones de \texttt{top}, \texttt{ps} y \texttt{/proc} son las que permiten
comprobar qué ocurrió realmente.
